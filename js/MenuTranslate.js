...
export function applyMenuTranslation(T) {
  try {
    ...
    texe.T = T;
    const isVueMode = window?.comfyAPI?.app?.app?.vueAppReady;

    if (isVueMode) {
      const vueRoot = document.querySelector("#vue-app") || document.body;
      texe.translateAllText(document.body);
      const obs = observeFactory(document.body, (mutationsList) => {
        for (let mutation of mutationsList) {
          mutation.addedNodes.forEach((n) => texe.translateAllText(n));
        }
      }, true);
